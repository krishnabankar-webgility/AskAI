...
- **`/wd-es-kibana`** — Elasticsearch log analyst: daily log reports, error investigation, health checks via Kibana WD HTTPS API or ES MCP. Posts report summaries to Slack `#wd_performance` via webhook (`wd-es-kibana.skill.md`, `slack-integration.skill.md`). Standalone script: `.mcp-servers/es-logs/fetch-daily-logs.mjs`.
