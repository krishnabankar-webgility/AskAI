...
5. **Render the Slack message containing ONLY §5 + §6** in the exact template from the skill's "Output format" section. Sections §1 Yesterday / §2 Today / §3 Pending / §4 Follow-ups are **computed internally** to derive §5's named sub-bullets and §6's next-action sentence — they are **not** rendered into the Slack message. Empty sub-bullet groups render `_(nothing)_` — never drop a header. Send as a **single** `slack_send_message` call; never split into "v2 / addendum" follow-ups. If Krishna explicitly asks for the long view, render §1–§4 in chat or post them as a thread reply on the day's main message — never as a new top-level message in `#my-daily-update`.
6. **Deliver via `slack_send_message` (parameter is `message`, not `text`):**
   - **Manual run** (user typed `/daily-work-update` in chat): show the full §5 + §6 draft in chat, then ask one confirm `"Post to #my-daily-update? (yes / no / DM only)"`. (Optionally show the internal §1–§4 computation in chat for Krishna's reference, but only the §5 + §6 message goes to Slack.)
   - **Scheduled run** (env `DAILY_UPDATE_AUTOSEND=1`): post the §5 + §6 message directly to channel `C0B0CBW8G03` (`#my-daily-update`). Fall back to **DM Krishna** by passing `U08FTS2SRAP` as `channel_id` if the channel does not resolve. Fall back to chat-only (still §5 + §6 only) if Slack MCP is unavailable.
