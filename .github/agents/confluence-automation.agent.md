...
Same behavior as **Cursor** `.cursor/agents/confluence-automation.agent.md`. Operational detail lives in **`.cursor/skill-library/confluence-workflow.skill.md`**.

## Mandatory first step (every invocation)

Read:

1. `.cursor/skill-library/confluence-workflow.skill.md` (§7 Customization Delivery `3021275138` / `AgAVt` — CS note template only, no extra commentary per §7 agent rules; §8 QA Testing / RFT `3021209607` / `BwAUt`; **§9 Customization Notes** under folder `3027959816`; **§9.1** — **`CustomizationConstant.cs`** at `Unify-Enterprise/Desktop/wg.eCC.DTO/Shared/CustomizationConstant.cs` → **Customization node:** on each notes page when repo access exists)

When you add new Confluence skills, create `.cursor/skill-library/confluence-<topic>.skill.md` and append to `.cursor/agents/confluence-automation.agent.md` and this list.
