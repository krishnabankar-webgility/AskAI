...
Same behavior as **Cursor** `.cursor/agents/slack-automation.agent.md`.
...
When adding Slack skills, create `.cursor/skill-library/slack-<topic>.skill.md` and append to `.cursor/agents/slack-automation.agent.md` and this list.

## After skills are loaded

1. Check whether the **Slack MCP server** is active (see `.cursor/mcp.json`). If not, guide setup per `slack-integration.skill.md`.
